...
public class OracleCDCSource extends BaseSource {
  ...
  private final Map<String, HashQueue<RecordSequence>> bufferedRecords = new HashMap<>();
  ...
  public String produce(String lastSourceOffset, int maxBatchSize, final BatchMaker batchMaker) throws StageException {
    ...
    while (!getContext().isStopped() && !recordsProduced) {
      if (currentCommitTxn.isPresent()) {
        ...
        Iterator<RecordSequence> txn = bufferedRecords.get(currentCommitTxn.get()).iterator();
        while (txn.hasNext() && txn.next().seq <= os.sequence) {
          txn.remove();
        }
        int newSeq = addRecordsToBatch(batchSize, batchMaker, os.scn, currentCommitTxn.get());
        ...
        LOG.info("Reached :" + offsetReference.get());
        continue;
      }
      ...
      try {
        ...
        if (!StringUtils.isEmpty(lastSourceOffset)) {
          ...
          if (lastSourceOffset.startsWith("v3")) {
            ...
            if (lastEndTime != null) {
              ...
              refreshed = false; // Don't look back, it causes duplicates to be read
            } else {
        ...
        try {
          if (refreshed && !configBean.bufferLocally) {
            startDate = startDate.minusSeconds(configBean.txnWindow);
          }
  ...
  @NotNull
  private LocalDateTime localDateTimeFromEpoch(long millis) {
    return LocalDateTime.ofInstant(
        Instant.ofEpochSecond(millis), TimeZone.getDefault().toZoneId());
  }
    ...
    try {
      while (resultSet.next()) {
        ...
        if (resultSet.getInt(9) == 0) {
          ...
          if (bufferedRecords.containsKey(xid) && bufferedRecords.get(xid).contains(new RecordSequence(null, 0, rsId, ssn))) {
            continue;
          }
          Offset offset;
          if (LOG.isDebugEnabled()) {
          ...
          if (op != DDL_CODE && op != COMMIT_CODE && op != ROLLBACK_CODE) {
            ...
            if (record.getEscapedFieldPaths().size() > 0) {
              if (configBean.bufferLocally) {
                HashQueue<RecordSequence> records =
                    bufferedRecords.computeIfAbsent(xid, x -> new HashQueue<>());
          ...
          } else if (op == COMMIT_CODE || op == ROLLBACK_CODE) {
            ...
            if (op == ROLLBACK_CODE || scnDecimal.compareTo(resumeCommitSCN) < 0) {
              ...
              if (scnDecimal.compareTo(resumeCommitSCN) == 0 && bufferedRecords.containsKey(xid)) {
                equal = true;
                Iterator<RecordSequence> txn = bufferedRecords.get(xid).iterator();
              ...
              int lastSeq = 0;
              int bufferedRecordsToBeRemoved = bufferedRecords.getOrDefault(xid, EMPTY_LINKED_HASHSET).size();
              if (bufferedRecordsToBeRemoved > 0) {

                lastSeq = addRecordsToBatch(batchSize, batchMaker, scn, xid);
                if (equal) {
      ...
      if (forceNewResultSet || countToCheck < batchSize) {
  ...
  private int addRecordsToBatch(int batchSize, BatchMaker batchMaker, String scn, String xid) {
    HashQueue<RecordSequence> records = bufferedRecords.getOrDefault(xid, EMPTY_LINKED_HASHSET);
  ...
  private class Offset {
    ...
    public Offset(String offsetString) {
      ...
      this.timestamp =
          version.equals("v3") ?
              LocalDateTime.ofInstant(Instant.ofEpochSecond(Long.parseLong(splits[1])), ZoneId.systemDefault()): null;
      this.scn = splits[2];
      this.sequence = Integer.parseInt(splits[3]);
    }
